/**
 * A class that contains properties necessary to perform ListJobs
 * @param {string} [compartmentId] The compartment OCID on which to filter.
 * @param {string} [stackId] The stack OCID on which to filter.
 * @param {string} [id] The OCID on which to query for jobs.
 * @param {string} [lifecycleState] A filter that returns all resources that match the specified lifecycle state. The state value is case-insensitive. Allowable values: ACCEPTED IN_PROGRESS FAILED SUCCEEDED CANCELING CANCELED
...
class ListJobsRequest {
	...
	constructor(compartmentId, stackId, id, lifecycleState, displayName, sortBy, sortOrder, limit, page, opcRequestId){
		this._headers = {};
		this._pathParams = {};
		this._queryParams = {};
		this.body = '';
	...
	get queryParams(){
		...
		this._queryParams['stackId'] = this.stackId;
...
module.exports = ListJobsRequest;